...
use entities::cloud_account::{Column as CloudAccountCloumn, Entity as CloudAccountEntity};
use reqwest::Client;
...
pub async fn get_shared_drives(
    ...
    match cloud_account {
        Ok(acc) => match acc {
            None => Err(AppError::Forbidden(Some(String::from(
                    ...
                    match res {
                        ...
                        Err(err) => {
                            ...
                            Err(AppError::Internal(Some(String::from(
                                "Error getting response form the google api",
                            ))))
                        }
                    }
                }
